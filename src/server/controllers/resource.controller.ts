...
export abstract class ResourceController<M extends AbstractModel> extends AbstractController {
  ...
  public async deleteOne(request: Request, response: Response): Promise<Response> {
    ...
    const requestData: any = await request.getPayload();
    const model: M = await this.modelStore.hydrate(requestData);
   ...
   * @returns {Promise<Response>}
   */
  protected async savePayload(request: Request, response: Response, checkExists: boolean = false, validatorOptions: ValidatorOptions = {}): Promise<Response> {
    const payloadData: any = await request.getPayload();
    let model: M = await this.modelStore.hydrate(payloadData);

    if (checkExists) {
      const modelExists: boolean = await this.modelStore.hasOne(model);
    ...
    const savedModel = await this.modelStore.saveOne(model);
    return response.data(savedModel);
  }
